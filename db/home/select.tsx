...
export async function getRecordsIncome( offset: number ) {
    try {
        const db = await getDB()
        const fetch = await db.getAllAsync<any>(`
                SELECT  typeId AS id,
        ...
        handleDBError(e, 'Fetching income records for home screen failed')
    }
}


export async function getRecordsExpenses( offset: number ) {
    try {
        const db = await getDB()
        const fetch = await db.getAllAsync<any>(`
                SELECT  typeId AS id,
        ...
        handleDBError(e, 'Fetching expense records for home screen failed')
    }
}


export async function getRecordsTransfer( offset: number ) {
    try {
        const db = await getDB()
        const fetch = await db.getAllAsync<any>(`
                SELECT  tr.transferId AS id,
